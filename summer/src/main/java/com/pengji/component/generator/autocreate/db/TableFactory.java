package com.pengji.component.generator.autocreate.db;
	...
	private Table createTable(Connection conn, ResultSet rs) throws SQLException {
		...
		try {
			...
			if(remarks == null || remarks.equals("")) {
	            ...
	            ResultSet createTableRs = stmt.executeQuery("SHOW CREATE TABLE " + realTableName);
	            if (createTableRs != null && createTableRs.next()) {
	                String create = createTableRs.getString(2);
	                remarks = parse(create);
	            }
	            createTableRs.close();
			}
			Table table = new Table();
