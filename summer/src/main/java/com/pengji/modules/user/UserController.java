 ...
 */
@ControllerBind(controllerKey = "/summer/user")
public class UserController extends BaseProjectController {
	...
	private static final String path = "/pages/modules/summer/user/user_";
	...
	public void list() {
		...
		if (model.getAttrValues().length != 0) {
			sql.setAlias("t");
			// 查询条件
			sql.whereEquals("loginName", getPara("loginName")); 
			sql.whereEquals("password", getPara("password")); 
			sql.whereEquals("name", getPara("name")); 
			sql.whereEquals("loginFlag", getPara("loginFlag")); 
		}
		// 排序
		...
		Page<SysUser> page = srv.paginate(getParaToInt("p", 1), 40);
		System.out.println(Json.getJson().toJson(page));
		// 下拉框
		setAttr("page", page);
		setAttr("attr", model);
